...
void kernel_symm(int m, int n,
		 ...
		 DATA_TYPE POLYBENCH_2D(A,M,M,m,m),
		 DATA_TYPE POLYBENCH_2D(B,M,N,m,n),
                 DATA_TYPE POLYBENCH_2D(_e_temp2,M,N,m,n))
{
  int i, j, k;
   ...
   for (i = 0; i < _PB_M; i++)
      for (j = 0; j < _PB_N; j++ )
      {
        ...
        for (k = 0; k < i; k++) {
           ...
           _e_temp2[i][j] += B[k][j] * A[i][k];
        }
        C[i][j] = beta * C[i][j] + alpha*B[i][j] * A[i][i] + alpha * _e_temp2[i][j];
     }
#pragma endscop
...
int main(int argc, char** argv)
{
  ...
  POLYBENCH_2D_ARRAY_DECL(_e_temp2,DATA_TYPE,M,N,m,n);
